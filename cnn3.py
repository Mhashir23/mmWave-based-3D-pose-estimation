...
split_train = int(0.8 * num_samples)  # 70% training
split_val = int(0.9 * num_samples)   # 15% validation, 15% testing
...
total_res.to_csv('cnn5res.csv', index=False)
...
# Save results to CSV
eval_results.to_csv('joint_wise_evaluation_with_totals5.csv', index=False)
...
plt.tight_layout()
plt.savefig('joint_wise_evaluation_with_totals5.png')
plt.show()


# Save the model and results
cnn_model.save('mars_cnn_model5.h5')
np.save('test_predictions5.npy', predictions)
np.save('test_ground_truth5.npy', y_test)
