...
split_train = int(0.6 * num_samples)  # 60% training
split_val = int(0.8 * num_samples)   # 20% validation, 20% testing
...
def build_model(hp):
    inputs = Input(shape=(9, 12, 5))
    
    # Convolutional layers
    conv1 = Conv2D(16, kernel_size=(3, 3), activation='relu', padding='same')(inputs)
    conv1 = Dropout(hp.Float('dropout_rate_1', 0.2, 0.5, step=0.1))(conv1)
    conv2 = Conv2D(32, kernel_size=(3, 3), activation='relu', padding='same')(conv1)
    conv2 = Dropout(hp.Float('dropout_rate_2', 0.2, 0.5, step=0.1))(conv2)
    conv2 = BatchNormalization(momentum=0.95)(conv2)
    
    # Flatten and dense layers
    flat = Flatten()(conv2)
    dense1 = Dense(hp.Int('units', min_value=256, max_value=512, step=128), activation='relu')(flat)
    dense1 = BatchNormalization(momentum=0.95)(dense1)
    dense1 = Dropout(0.4)(dense1)
    
    # Output layer
    outputs = Dense(train_labels.shape[1], activation='linear')(dense1)
...
tuner = kt.BayesianOptimization(
    build_model,
    objective='val_loss',
    max_trials=20,  # Maximum number of hyperparameter combinations
    directory='bayesian_tuning',
    project_name='cnn_pose_estimation'
)
...
best_model.save('best_mars_cnn_model.h5')
...
total_res.to_csv('cnn4res.csv', index=False)
...
num_joints = train_labels.shape[1] // 3
...
# Save results to CSV
eval_results.to_csv('joint_wise_evaluation_with_totals4.csv', index=False)

# Display joint-wise evaluation including totals
print("Joint-wise Evaluation with Total MAE and RMSE:")
print(eval_results)

# Visualization: Plot joint-wise MAE and RMSE
...
plt.tight_layout()
plt.savefig('joint_wise_evaluation_with_totals4.png')
plt.show()

# Save predictions and ground truth for further analysis
np.save('test_predictions4.npy', predictions)
np.save('test_ground_truth4.npy', y_test)
